##-----------------------
#' # Template code for replicating simulation setting III in the main paper
...
sim_fn <- function(seed = NULL,
    ...
    simdat$f <- simdat$f1
    simdat$y <- rnorm(N, mean = simdat$f, sd = sqrt(true_sigma2))

    #' ## Create outliers by finding the intercept value c such that mean(plogis(5 x (x1-c))) = prob_outlier.
    #' *Note this will change with dataset!*
    if(prob_outlier == 0)
        find_intercept <- list(root = Inf)
    if(prob_outlier > 0)
        find_intercept <- uniroot(f = function(x) mean(plogis(5*(simdat$x1 - x))) - prob_outlier, interval = c(-100,100))
    create_outliers <- rbinom(nrow(simdat), size = 1, prob = plogis(5*(simdat$x1 - find_intercept$root)))
    ...
    simdat_test$y <- rnorm(1000, mean = simdat_test$f, sd = sqrt(true_sigma2))
    rm(find_intercept)
