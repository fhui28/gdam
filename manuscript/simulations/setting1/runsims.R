##-----------------------
#' # Template code for replicating simulation setting I in the main paper
...
sim_fn <- function(seed = NULL,
    ...
    simdat <- data.frame(x = runif(N)) %>%
        ...
        mutate(y = rnorm(nrow(.), mean = eta, sd = 1))

    create_outliers <- rbinom(nrow(simdat), size = 1, prob = prob_outlier)
    table(create_outliers)
    simdat$contaminated_y <- simdat$y
    simdat$contaminated_y[create_outliers == 1] <- rnorm(sum(create_outliers == 1), mean = simdat$eta[create_outliers==1], sd = sqrt(true_sigma2)*9)

    simdat_test <- data.frame(x = runif(1000)) %>%
        mutate(eta = 1.8 * sin(3.4 * x^2)) %>%
        mutate(y = rnorm(nrow(.), mean = eta, sd = sqrt(true_sigma2)))
...
num_datasets <- 200
cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, prob_outlier = 0, true_sigma2 = 1)
save(cw_results, file = "setting1_N200_sigma21_prob00.RData")
rm(cw_results)

cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, prob_outlier = 0.05, true_sigma2 = 1)
save(cw_results, file = "setting1_N200_sigma21_prob005.RData")
rm(cw_results)

cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, prob_outlier = 0.1, true_sigma2 = 1)
save(cw_results, file = "setting1_N200_sigma21_prob01.RData")
rm(cw_results)


cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, prob_outlier = 0, true_sigma2 = 2)
save(cw_results, file = "setting1_N200_sigma22_prob00.RData")
rm(cw_results)

cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, prob_outlier = 0.05, true_sigma2 = 2)
save(cw_results, file = "setting1_N200_sigma22_prob005.RData")
rm(cw_results)

cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, prob_outlier = 0.1, true_sigma2 = 2)
save(cw_results, file = "setting1_N200_sigma22_prob01.RData")
rm(cw_results)


cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, N = 400, prob_outlier = 0, true_sigma2 = 1)
save(cw_results, file = "setting1_N400_sigma21_prob00.RData")
rm(cw_results)

cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, N = 400, prob_outlier = 0.05, true_sigma2 = 1)
save(cw_results, file = "setting1_N400_sigma21_prob005.RData")
rm(cw_results)

cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, N = 400, prob_outlier = 0.1, true_sigma2 = 1)
save(cw_results, file = "setting1_N400_sigma21_prob01.RData")
rm(cw_results)


cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, N = 400, prob_outlier = 0, true_sigma2 = 2)
save(cw_results, file = "setting1_N400_sigma22_prob00.RData")
rm(cw_results)

cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, N = 400, prob_outlier = 0.05, true_sigma2 = 2)
save(cw_results, file = "setting1_N400_sigma22_prob005.RData")
rm(cw_results)

cw_results <- foreach(l0 = 1:num_datasets) %dopar% sim_fn(seed = l0, N = 400, prob_outlier = 0.1, true_sigma2 = 2)
save(cw_results, file = "setting1_N400_sigma22_prob01.RData")
rm(cw_results)
