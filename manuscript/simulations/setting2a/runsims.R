##-----------------------
#' # Template code for replicating simulation setting IIa in the main paper
...
sim_fn <- function(seed = NULL,
    ...
    if(distribution == "t") {
        simdat$y <- simdat$f + rt(nrow(simdat), df = dist_param)
        simdat$contaminated_y <- simdat$y #rnorm(nrow(simdat), mean = simdat$f2, sd = 2)
        }
    if(distribution == "laplace") {
        simdat$y <- simdat$f + VGAM::rlaplace(nrow(simdat), scale = dist_param)
